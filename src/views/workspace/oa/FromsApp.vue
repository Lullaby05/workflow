<template>
  <div v-loading="loading">
    <div class="fixed-search hidden-xs-only">
      <el-input clearable placeholder="搜索表单" prefix-icon="el-icon-search" v-model="searchForm"></el-input>
    </div>
    <el-row :gutter="20">
    ...
    <div class="group" v-if="searchForm && searchForm.trim() !== ''">
      ...
      <div class="group-container">
        <div class="group-item" v-for="(item, index) in searchResult" :key="index" @click="enterItem(item)">
          <div>
            <icon :name="item.logo.icon" :style="'background: ' + item.logo.background"></icon>
    ...
    <div v-else>
      <div class="group" v-if="recentlyUsed && recentlyUsed.length > 0">
        <div class="group-title">
          最近使用
          <el-link style="float: right" :underline="false" link icon="el-icon-delete" @click="clearUsed">清空</el-link>
        </div>
        <div class="group-container">
    ...
    <w-dialog v-if="!isMobile" :title="`发起 - ${selectForm.formName}`" closeFree width="1000px" v-model="openItemDl" okText="提 交" @cancel="openItemDl = false" @ok="submitForm">
      <initiate-process ref="processForm" :code="selectForm.formId" v-if="openItemDl" @ok="openItemDl = false"></initiate-process>
    </w-dialog>
  </div>
</template>
