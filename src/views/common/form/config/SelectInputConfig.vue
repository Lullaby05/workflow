      ...
      <el-form-item label="选项设置" class="options">
        <template #label>
          <div class="option-item-label">
            <span>选项设置</span>
            <el-button icon="el-icon-plus" link size="small" @click="modelValue.options.push('新选项')">新增选项</el-button>
          </div>
        </template>
        <draggable item-key="id" v-model="modelValue.options" :component-data="{ tag: 'div', type: 'transition-group' }" handler=".el-icon-rank" v-bind="dragOption">
          <template #item="{ element, index }">
            <div class="option-item">
              <icon name="el-icon-rank"></icon>
              <el-input v-if="modelValue.isNeedApi" v-model="modelValue.options[index].label" placeholder="请设置选项名称" clearable :disabled="modelValue.isNeedApi">
                <template #append>
                  <el-button icon="el-icon-delete" type="danger" @click="modelValue.options.splice(index, 1)"></el-button>
                </template>
              </el-input>
              <el-input v-else v-model="modelValue.options[index]" placeholder="请设置选项值" clearable :disabled="modelValue.isNeedApi">
                <template #append>
                  <el-button icon="el-icon-delete" type="danger" @click="modelValue.options.splice(index, 1)"></el-button>
                </template>
              </el-input>
