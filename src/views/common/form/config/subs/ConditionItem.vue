<template>
  <div>
    <el-select style="width: 150px; margin: 5px" @change="fieldChange" value-key="id" v-model="_value.field" placeholder="选择表单字段">
    ...
    <template v-if="_value.field">
      ...
      <el-select v-else style="width: 150px; margin: 5px" value-key="id" v-model="_value.compareVal[0]" placeholder="选择表单字段">
        <el-option :key="item.id" v-for="item in _formItems" :value="item.id" :label="item.title"></el-option>
      </el-select>
    </template>
