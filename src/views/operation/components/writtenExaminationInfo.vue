<template>
  <div class="form-render-container">
      ...
      <div style="text-align: right; display: flex">
        ...
        <Button
          v-if="
            type === 'detail' &&
            currentProcess &&
            props.btnPermission.includes('Review')
          "
          type="primary"
          size="mini"
          @click="jumpToHandle"
    ...
    >
      <div class="audit-form-title">{{ '书面审查意见' }}</div>
    ...
    <div class="audit-form-container">
      <div class="audit-form-container-title">{{ '书面审查情况' }}</div>
    ...
    >
      <div class="audit-item-title">
        <span class="audit-item-title-text">{{
          '第' + (historyData.length - index) + '次审查结果'
        }}</span>
      </div>
      <div class="audit-content">
        <detailInfo
  ...
  for (let i = props.formProcessData.progress.length - 1; i > 0; i--) {
    ...
    if (item.processKey === 'review') {
      ...
      if (
        ...
        props.formProcessData.progress[i - 1].processKey !== 'review'
      ) {
        return false;
      }
    }
...
for (let i = 0; i < tempProgress.length; i++) {
  if (tempProgress[i].processKey === 'review') {
    processData.push(tempProgress[i]);
  }
}

// 只有书面审核中并且需要他签名的情况才可以编辑
const status = computed<string>(() => {
  ...
  if (condition && currentProcess.value && props.type === 'edit') {
    return 'edit';
  }
  return 'detail';
});
