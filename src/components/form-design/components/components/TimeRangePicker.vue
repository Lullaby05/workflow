  ...
  export default {
    ...
    mounted() {
      if (Array.isArray(this.modelValue) && this.modelValue.length > 0) {
        this.timeRange = [
          this.$isNotEmpty(this.modelValue[0])
            ? this.modelValue[0]
            : '00:00:00',
          this.$isNotEmpty(this.modelValue[1])
            ? this.modelValue[0]
            : '00:00:00',
        ];
      }
    ...
    methods: {
      ...
      onConfirm(val) {
        ...
        if (this._value[0] && this._value[1]) {
          if (
            !moment(date + this._value[0]).isBefore(
              moment(date + this._value[1])
            )
          ) {
            this._value[this.valIndex] =
              this.valIndex === 0 ? '00:00:00' : this._value[0];
