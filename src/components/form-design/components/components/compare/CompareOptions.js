...
const strArrCompareOptions = [
  {
    name: '包含',
    symbol: 'HS',
    compare: (a, b) => checkElementsExistInArray(b, a),
  },
  {
    name: '不包含',
    symbol: 'NHS',
...
const orgCompareOptions = [
  ...
  { name: '不是', symbol: 'NEQ', compare: (a, b) => a[0].id != b[0].id },
  {
    name: '为其之一',
    symbol: 'IN',
    compare: (a, b) => checkElementsExistInArray(a, b, 'id'),
  },
  {
    name: '含有',
    symbol: 'HS',
    compare: (a, b) => checkElementsExistInArray(b, a, 'id'),
  },
  {
    name: '不含有',
    symbol: 'NHS',
...
const timeCompareOptions = [
  {
    name: '在之前',
    symbol: 'LT',
    compare: (a, b) => moment(a).isBefore(moment(b[0])),
  },
  {
    name: '在之后',
    symbol: 'GT',
    compare: (a, b) => moment(a).isAfter(moment(b[0])),
  },
  {
    name: '在其中',
    symbol: 'IN',
    compare: (a, b) =>
      moment(b[0]).isBefore(moment(a)) && moment(b[1]).isAfter(moment(a)),
  },
];
...
const timeArrCompareOptions = [
  {
    ...
    symbol: 'HS',
    compare: (a, b) =>
      moment(a[0]).isBefore(moment(b[0])) && moment(a[1]).isAfter(moment(b[0])),
  },
  {
    name: '不包含',
    symbol: 'NHS',
    compare: (a, b) =>
      moment(b[0]).isBefore(moment(a[0])) || moment(b[0]).isAfter(moment(a[1])),
  },
];
...
function getCompareFucs() {
  ...
  timeCompareOptions.forEach(
    (v) => (cpFuncs.timeCompare[v.symbol] = v.compare)
  );
  timeArrCompareOptions.forEach(
    (v) => (cpFuncs.timeArrCompare[v.symbol] = v.compare)
  );
  return cpFuncs;
}
