<template>
  <div>
    ...
    <w-dialog
      title="配置计算公式"
      :width="modelValue.isPlus ? '760px' : '560px'"
      v-model="visible"
      @opened="init"
      @ok="validExplain"
    >
        ...
        <el-form class="calc-keys">
          <el-form-item label="表单变量:">
            <div v-if="formVars.length > 0">
              <div
                @click="appendExplain(item)"
                class="calc-tag var-tag"
                v-for="item in formVars"
                :key="item.symbol"
              >
                {{ item.name }}
              </div>
      ...
      <div v-else>
        <el-alert
          type="info"
          description="🎈 使用js构建任意计算规则，取表单字段值（formData.字段ID）"
          :closable="false"
        />
        <div style="margin-top: 10px">
          ...
          <template v-if="formAndTableVars.length > 0">
            <el-tooltip
              :content="copyOk ? '复制成功√' : '点击复制变量'"
              placement="top"
              v-for="item in formAndTableVars"
              :key="item.symbol"
            >
              <div @click="copyVar(item)" class="calc-tag">{{ item.name }}</div>
            </el-tooltip>
          </template>
          <template v-else style="color: #8c8c8c"
            >表单内数值类型的字段才可作为变量参与计算</template
          >
        </div>
        <code-editor
          ref="code"
    ...
    methods: {
      ...
      modeChange() {
        ...
        if (this.modelValue.isPlus) {
          this.jsCodeTemp =
            this.modelValue.jsCode.trim().length > 0
